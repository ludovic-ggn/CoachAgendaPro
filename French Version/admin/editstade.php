...
include '../config.php';
  ...
  $result=mysqli_query($conn, $sql);
  $row=mysqli_fetch_assoc($result);
  $nomstade=$row['nomstade'];
  $adresse=$row['adresse'];
  $ville=$row['ville'];

  if (isset($_POST['submit'])) {
    $nomstade=mysqli_real_escape_string($conn, $_POST['nomstade']);
    $adresse=mysqli_real_escape_string($conn, $_POST['adresse']);
    $ville=mysqli_real_escape_string($conn, $_POST['ville']);

    $adressecomplete= $adresse.", ".$ville;

    // Construction de l'URL de requête HTTP
    $url = 'https://dev.virtualearth.net/REST/v1/Locations/' . urlencode($adressecomplete) . '?key=XXXXXXXXXXXXXXXXXXXX';

    // Envoi de la requête HTTP à l'API de géocodage Bing Maps
    $reponse = file_get_contents($url);

    // Analyse de la réponse JSON pour extraire les coordonnées géographiques
    $donnees = json_decode($reponse, true);
    $latitude = $donnees['resourceSets'][0]['resources'][0]['point']['coordinates'][0];
    $longitude = $donnees['resourceSets'][0]['resources'][0]['point']['coordinates'][1];

    $gps= $latitude.", ".$longitude;

    $sql="UPDATE stades set nomstade='$nomstade', adresse='$adresse', ville='$ville', gps='$gps' WHERE id=$id";
    $result=mysqli_query($conn, $sql);
    if ($result) {
...
>
  <head>
    ...
    <title>Mofication d'un stade - <?php echo $nomclub;?> - CoachAgendaPro</title>
        ...
        <div class="layout-page">
          ...
          <div class="content-wrapper">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Espace coach/</span> Modification d'un stade</h4>
                ...
                <div class="col-xl">
                  <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                      <h5 class="mb-0">Modification d'un stade</h5>
                    </div>
                    <div class="card-body">
                      <form method="post">
                        <div class="mb-3">
                          <label class="form-label">Nom du stade</label>
                          <input type="text" class="form-control" name="nomstade" value="<?= htmlspecialchars($nomstade); ?>" required />
                        </div>
                        <div class="mb-3">
                          <label class="form-label">Adresse</label>
                          <input type="text" class="form-control" name="adresse" value="<?= htmlspecialchars($adresse); ?>" required />
                        </div>
                        <div class="mb-3">
                          <label class="form-label">Ville</label>
                          <input type="text" class="form-control" name="ville" value="<?= htmlspecialchars($ville); ?>" required />
                        </div>
